...
def check_fwd_kernel(has_bias=False):
    ...
    attn_output = np.genfromtxt("attn_output.data", delimiter=" ", dtype=np.float32)
...
def check_bwd_kernel(has_bias=False):
    ...
    attn_dq = np.genfromtxt("attn_dq.data", delimiter=" ", dtype=np.float32)
    attn_dk = np.genfromtxt("attn_dk.data", delimiter=" ", dtype=np.float32)
    attn_dv = np.genfromtxt("attn_dv.data", delimiter=" ", dtype=np.float32)
